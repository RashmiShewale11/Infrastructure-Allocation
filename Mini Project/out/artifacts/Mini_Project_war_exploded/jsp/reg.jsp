...
<%
    ...
    try {
        ...
        if("A3-001".equals(lab) || "A3-002".equals(lab) || "A1-004".equals(lab) ||"F-001".equals(lab))
        {
            PreparedStatement stmt1 = con.prepareStatement("select assistant_id from Seminar_Halls where hall_no=?");
            ...
            PreparedStatement stmt = con.prepareStatement("insert into events (lab_no, mis_id, assistant_id, stud_roll, stud_name, stud_email, mob_no, dept, description, date, start_time, end_time, flag, event_name)values (?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
            ...
            stmt.setInt(13, flag);
            stmt.setString(14, ename);
            count = stmt.executeUpdate();

            if(count>0) {

                String proc = "{call add_to_time()}";
            ...
            PreparedStatement stmt1 = con.prepareStatement("select assistant_id from Labs where lab_no=?");
            stmt1.setString(1, lab);
            ResultSet rs1 = stmt1.executeQuery();
            while (rs1.next()) {
                lab_assi_id = rs1.getString("assistant_id");
